...
const SQL_CASHOUT = "INSERT INTO settlement(bet_id, lobby_id, user_id ,operator_id, name, bet_amount, auto_cashout, balance, max_mult, status) VALUES(?,?,?,?,?,?,?,?,?,?)"
...
const SQL_INSERT_BETS = 'INSERT INTO bets (bet_id, lobby_id, user_id, operator_id, name, balance, bet_amount, auto_cashout) VALUES(?,?,?,?,?,?,?,?)';

const addCashout = async (data) => {
    try {
        const { name, balance, lobby_id, bet_amount, user_id, operator_id, max_mult, bet_id, maxAutoCashout } = data;
        const autoCashout = maxAutoCashout === 'null' ? null : maxAutoCashout;
        await write(SQL_CASHOUT, [bet_id, lobby_id, decodeURIComponent(user_id), operator_id, name, bet_amount, autoCashout, balance, max_mult, "cashout"])
...
const addSettleBet = async (data) => {
    try {
        ...
        const finalData = [];
        for (let x of data) {
            const { lobby_id, bet_amount, user_id, operator_id, bet_id, name, balance, max_mult, maxAutoCashout } = x;
            const autoCashout = maxAutoCashout === 'null' ? null : maxAutoCashout;
            finalData.push([bet_id, lobby_id, decodeURIComponent(user_id), operator_id, name, bet_amount, autoCashout, balance, max_mult])
        }
        const placeholders = finalData.map(() => '(?, ?, ?, ?, ?, ?, ?, ?,?)').join(',');
        const SQL_SETTLEMENT = ` INSERT INTO settlement  (bet_id, lobby_id, user_id, operator_id, name, bet_amount, auto_cashout, balance, max_mult)  VALUES ${placeholders}`;
...
const insertBets = async (betData) => {
    try {
        let { bet_id, name, balance, maxAutoCashout } = betData;
        ...
        await write(SQL_INSERT_BETS, [bet_id, lobby_id, decodeURIComponent(user_id), operator_id, name, balance, bet_amount, maxAutoCashout]);
